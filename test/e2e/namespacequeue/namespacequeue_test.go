...
var _ = Describe("NamespaceQueue", func() {
	...
	It("keeps same-named NamespaceQueues isolated across namespaces", func() {
		fixture := newNamespaceQueueFixture()
		otherNamespace := uniqueName("nq-isolation")
		...
		name := uniqueName("shared-name")
		first := newNamespaceQueue(fixture.ctx.Namespace, name, "cluster/"+queueName)
		second := newNamespaceQueue(otherNamespace, name, "cluster/"+queueName)
		...
		Expect(waitNamespaceQueue(&otherCtx, name, func(queue *schedulingv1beta1.NamespaceQueue) bool {
			return apiMeta.FindStatusCondition(queue.Status.Conditions, commonutil.NamespaceQueueReadyCondition).Status == metav1.ConditionTrue
		})).NotTo(HaveOccurred())
	})
